...
| card_count | 카드 장수 | 7 | O |
| template_style | 사용한 비주얼 템플릿 이름 (승인된 고정 템플릿 파일을 참조 -- 매번 새로 디자인하지 않음) | default_v1 | O |
| character_cast | 이 세트에 등장하는 캐릭터 조합 (고정 자산 재사용, 재생성 안 함) | ["client", "lawyer"] | O |
...
- **character_cast를 CardSet에 둔 이유**: 캐릭터 이미지는 매번 새로 생성하지 않고 `templates/characters/`의 고정 자산을 재사용한다 (01_PRD.md 가정 원장 #15). 어떤 캐릭터 조합을 썼는지 기록해두면, 나중에 캐릭터 이미지 자체를 교체/추가할 때 영향받는 CardSet을 역추적할 수 있다.

---

## [NEEDS CLARIFICATION]

> 카드 장수(7장)와 캐릭터 배정 규칙은 확정됨 (01_PRD.md 가정 원장 #3, #15 참고). 남은 미결 사항 없음.
